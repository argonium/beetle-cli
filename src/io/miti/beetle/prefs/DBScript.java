...
public final class DBScript
{
   ...
   */
  private void process(final BufferedWriter writer,
                       final Connection conn)
  {
   ...
   */
  private Map<String, Map<IndexData, List<IndexColData>>>
    buildIndexData(final Connection conn, final List<String> tables)
  {
    // Build the full map of table index data
    ResultSet rs = null;
    Map<String, Map<IndexData, List<IndexColData>>> map =
      new HashMap<String, Map<IndexData, List<IndexColData>>>(10);
    try
    {
      ...
      for (String table : tables)
      {
        ...
        Map<IndexData, List<IndexColData>> childMap =
          new HashMap<IndexData, List<IndexColData>>(10);
        while (rs.next())
        {
          ...
          // Create the index object and append the column info object to its list
  ...
  private void printIndexData(final Map<String, Map<IndexData, List<IndexColData>>> map)
  {
    if (map == null)
    {
    ...
    for (Entry<String, Map<IndexData, List<IndexColData>>> entrySet : map.entrySet())
    {
    ...
    for (String table : tables)
    {
      try
      {
        ...
        while (rs.next())
        {
          ...
          sb.append(sbCols).append(") VALUES (")
            .append(sbValues).append(");").append(lineSep);
    ...
    for (String table : tables)
    {
      ...
      try
      {
        ...
        for (int i = 1; i <= numColumns; i++)
        {
          ...
          boolean nullable =
            (rsmd.isNullable(i) == java.sql.ResultSetMetaData.columnNullable);
          if (!nullable)
          {
            sb.append(" not");
          }
   ...
   */
  private void writeIndexData(final String table,
                               final BufferedWriter writer,
    final Map<String, Map<IndexData, List<IndexColData>>> indexData)
  {
    ...
    for (Entry<IndexData, List<IndexColData>> set : map.entrySet())
    {
      ...
      sb.append("index if not exists ").append(index.getName())
   ...
   */
  private String getPrimaryKeyName(final Connection conn,
                                   final String table)
  {
   ...
   */
  private String getColumnType(final ResultSetMetaData rsmd,
                               final int colNum)
  {
    try
    {
      ...
      if (bIncludeSize)
      {
        ...
        if (bIncludeDecimals)
        {
          sb.append("(").append(Integer.toString(colSize))
            .append(", ").append(Integer.toString(colPrecision))
            .append(")");
        }
        else
        {
          sb.append("(").append(Integer.toString(colPrecision))
            .append(")");
        }
      }
   ...
   */
  private void writeOut(final BufferedWriter writer,
                        final String msg)
  {
