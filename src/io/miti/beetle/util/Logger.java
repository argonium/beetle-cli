...
public final class Logger
{
  ...
  /**
   * Initialize the logging mechanism. The log level must be between 0 and 5.
   * Zero turns off logging, and 1-5 are more progressive logging levels.
   * 1=info, 2=debug, 3=warn, 4=error and 5=fatal. The logFile is the name of
   * the output file. File names of stdout and stderr redirect output to the
   * standard output stream and the standard error stream, respectively. Log
   * overwrite will overwrite the log file at startup if true; otherwise, it
   * will append.
   * 
   ...
   */
  public static void initialize(final int nLogLevel, final String sLogFile,
   ...
   */
  private static void writeMsg(final Exception ex, final int levelBound) {
  ...
  private static void writeMsg(final Exception ex, final int levelBound,
    ...
    if (elements.length > 0) {
      ...
      int lastElement = (depth < 0) ? elements.length : Math.min(
          elements.length, depth);
      for (int i = 1; i < lastElement; ++i) {
        sb.append(Utility.getLineSeparator()).append("    ")
            .append(elements[i]);
      }
    }
   ...
   */
  private static void writeMsg(final String msg, final int levelBound) {
    // Check if any messages should be written
    if ((logger.logLevel < 1) || (logger.logFileMode < 0)
